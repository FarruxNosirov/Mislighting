...
import {loadFavorite} from '@store/slices/favoriteSlice';
...
const Products = ({item}: {item: ProductItemResponse}) => {
  let {photo, name, price, discount, id, category} = item;
  ...
  const onCartPress = async () => {
    try {
      if (isInCart) {
        ...
        setAnimate(true);
        await requests.products.addToCart({
          amount: 1,
          product_id: id,
        });
        let cartRes = await requests.products.getCarts();
  ...
  const effect = async () => {
    try {
      await requests.favorites.getFavorites();
